...
export default class PendingJobs extends Component {
    ...
    _retrieveData = async () => {
        ...
        try {
          ...
          axios.get(`http://production.myquickshift.com/app_api/pending_jobs_api/${this.state.user_id}`)
          .then((response) => {
            //console.log(response.data, "------> console log Decline Jobs")
            this.setState({
                isLoading: false,
                data: response.data
           })
      ...
      list = () => {
        return this.state.data.pending_jobs.map((element, i) => {
          return (
            ...
            <CardItem style={{marginTop: '-3%', borderRadius: 10}}>
                <View style={styles.thumbTextStyle}>
                    <View style={{flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
                            ...
                            <Badge style={{ backgroundColor: '#C53431', height: 20, width: 90, alignItems: 'center' }}>
                                <Text style={{ color: 'white' }}>{element.job_active_status === 'ReadyForAdminApproval' ? "Pending" : ""}</Text>
                            </Badge>
