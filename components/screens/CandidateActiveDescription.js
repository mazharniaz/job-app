...
export default class CandidateActiveDescription extends Component {
    constructor(props) {
        super(props);
        this.state = {
            isLoading: true,
    ...
    componentDidMount() {
        this._retrieveCandidateJobData();
    }

    _retrieveCandidateJobData = async (id, user_id) => {
        
        try {
          const candidate = await AsyncStorage.getItem('CandidateJob');
          ...
          .then((response) => {
            console.log(response.data.image_path, "------> console log Active Candidate Jobs")
            this.setState({
                isLoading: false,
                data: response.data,
          ...
          console.log(error,"------> console log Active Candidate jobs retrieve error");
        });
    ...
    checkIn_API() {
        axios.get(`http://production.myquickshift.com/app_api/check_in_api/${this.state.user_id}/${this.state.id}/${this.state.user_id_2}`)
          .then((response) => {
            console.log(response.data, "------> console log Active Candidate Jobs")
            this.setState({
                isLoading: false,
                checkIn: response.data,
           })
           if(this.state.checkIn.check_in === "Check-In Successfuly.") {
    ...
    checkOut_API() {
        axios.get(`http://production.myquickshift.com/app_api/check_out_api/${this.state.lastCheckinID}`)
          .then((response) => {
            console.log(response.data, "------> console log Active Candidate Jobs")
            this.setState({
                isLoading: false,
        ...
        return (
            <Container>
                ...
                <Card transparent style={{marginTop: "-3%", borderBottomWidth: 1, marginLeft: "4%", marginRight: "4%"}}>
                    ...
                    <View style={{marginBottom: '1%', marginTop: '2%',}}>
                
                        <TouchableOpacity style={[styles.button, {marginLeft: '2%', marginRight: '2%'}]} onPress={() => this.handleCheckInButton()}>
                            <LinearGradient style={styles.signIn} colors={['#abec9e', '#0066ff']} start={{x: 0, y: 0}} end={{x: 1, y: 0}}>
                                <Text style={[styles.textSign, {color: '#ffffff'}]}>
                                    {!this.state.checkedInButton === true ? 'Check In' : 'Check out'}
                                </Text>
                            </LinearGradient>
                        </TouchableOpacity>
