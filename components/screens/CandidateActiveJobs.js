...
class CandidateActiveJobs extends Component {
    constructor(props) {
        super(props);
        this.state = {
            isLoading: true,
    ...
    _retrieveData = async () => {
        ...
        try {
          ...
          axios.get(`http://production.myquickshift.com/app_api/WorkFlow_api/${this.state.user_id}?status=Active`)
          .then((response) => {
            console.log(response.data, "------> console log Candidate Active Jobs")
            this.setState({
                isLoading: false,
                data: response.data
           })
        }, (error) => {
          console.log(error,"------> console log Candidate Active jobs error");
        });
...
export default CandidateActiveJobs
